...
function noQsMethod(options) {
  ...
  return function (socket) {
    ...
    socket.on('authenticate', function (data) {
      ...
      getSecret(socket.request, options.secret, options.completeToken, token, onSecretReady);
    });
  };
}
...
function getSecret(request, secret, completeToken, token, callback) {
  if (typeof secret === 'function') {
    ...
    var decodedToken = jwt.decode(token, { complete: completeToken || false });
